...
public class ContentSpecProcessor implements ShutdownAbleApp {
     ...
     * @param elm               An Error Logger Manager that will be used to store all the log messages in case they need to be accessed
     *                          at a later stage.
    ...
    protected void downloadAllTopics() {
        ...
        if (processingOptions.isUpdateRevisions() && processingOptions.getRevision() == null) {
            ...
            if (!topicIds.isEmpty()) {
                ...
                reader.getTopicsByIds(topicIds, csp.getContentSpec().getLocale() != null && !csp.getContentSpec().getLocale().equals(
    ...
    protected void downloadRevisionTopics(final List<Pair<Integer, Integer>> referencedRevisionTopicIds) {
        ...
        final boolean expandTranslations = csp.getContentSpec().getLocale() != null && !csp.getContentSpec().getLocale().equals(
                CommonConstants.DEFAULT_LOCALE);
    ...
    protected RESTTopicV1 createTopicEntity(final SpecTopic specTopic) throws Exception {
        ...
        try {
            ...
            if (specTopic.isTopicANewTopic()) {
                ...
                topic.setConfiguredParameters(
                        CollectionUtilities.toArrayList(RESTTopicV1.TAGS_NAME, RESTTopicV1.SOURCE_URLS_NAME, RESTTopicV1.INCOMING_NAME,
                                RESTTopicV1.OUTGOING_NAME, RESTTopicV1.PROPERTIES_NAME, RESTTopicV1.TITLE_NAME, RESTTopicV1.XML_NAME,
                                RESTTopicV1.DESCRIPTION_NAME, RESTTopicV1.HTML_NAME));
                ...
                for (final RESTTopicSourceUrlCollectionItemV1 sourceUrlItem : topic.getSourceUrls_OTM().getItems()) {
                    ...
                    sourceUrl.setConfiguredParameters(
                            CollectionUtilities.toArrayList(RESTTopicSourceUrlV1.TITLE_NAME, RESTTopicSourceUrlV1.URL_NAME,
                                    RESTTopicSourceUrlV1.DESCRIPTION_NAME));
                }
