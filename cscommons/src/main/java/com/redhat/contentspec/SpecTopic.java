...
public class SpecTopic extends SpecNode {
	...
	public boolean isInlineTopic() {
		return inlineTopic;
	}

	public void setInlineTopic(boolean inlineTopic) {
		this.inlineTopic = inlineTopic;
	}
	...
	public String getText() {
		if (text == null) {
			...
			if (DBId == 0) {
				...
				output += (isInlineTopic() ? "Inline: " : "") + title + " [" + id + ", " + type + (options.equals("") ? "" : (", " + options)) +  "]";
			} else {
				output += (isInlineTopic() ? "Inline: " : "") + title + " [" + id + "]";
			}
			if (!getRelatedRelationships().isEmpty()) {
				...
				output += " [R: " + StringUtilities.join(relatedIds.toArray(new String[0]), ", ") + "]";
			}
			if (!getPrerequisiteRelationships().isEmpty()) {
				...
				output += " [P: " + StringUtilities.join(relatedIds.toArray(new String[0]), ", ") + "]";
			}
			setText(output);
			return output;
		} else {
